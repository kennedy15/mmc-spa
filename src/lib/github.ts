...
export async function dispatchAddCharacter(token: string, inputs: { name: string; role: string; owner: string; world: string }): Promise<string | null> {
  const r = await gh<{ message?: string }>(token, `/repos/${REPO.owner}/${REPO.repo}/actions/workflows/${REPO.workflow}/dispatches`, {
    method: 'POST',
    body: JSON.stringify({ ref: REPO.branch, inputs }),
  });
  if (r.ok) return null;
  if (r.status === 401) return 'GitHub rejected the token.';
  if (r.status === 403 || r.status === 404) return 'The token cannot run workflows on this repo. It needs Actions: read and write.';
  return r.data?.message ?? `GitHub returned HTTP ${r.status}.`;
}
