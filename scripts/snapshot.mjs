...
async function fetchRanking(params) {
  ...
  const res = await fetch(url, { headers: { accept: 'application/json', 'user-agent': 'maple-tracker/1.0 (+github actions; 2 requests per character per day)' } });
...
async function lookupCharacter(name, worldId) {
  ...
  if (!row) return null;
  await sleep(GAP);
  // Legion ranking for the same world carries legionLevel / raidPower.
  let legion = null;
  try {
    const l = await fetchRanking({ type: 'legion', id: worldId, reboot_index: 0, page_index: 1, character_name: name });
    legion = (l.ranks ?? []).find((r) => r.worldID === worldId) ?? null;
  } catch (e) {
  ...
  return {
    ...
    legionLevel: legion?.legionLevel ?? null,
    legionRank: legion?.rank ?? null,
    raidPower: legion?.raidPower ?? null,
...
async function run() {
  ...
  for (const c of characters) {
    ...
    console.log(`Lv.${row.level} ${row.job}, legion ${row.legionLevel ?? '?'}`);
